...
bool32 IsDamageMoveUnusable(u32 move, u32 battlerAtk, u32 battlerDef)
{
    ...
    if (AI_BATTLER_HAS_TRAIT(battlerDef, ABILITY_SOUNDPROOF)
     && gMovesInfo[move].soundMove)
    {
        return TRUE;
    }
    if (AI_BATTLER_HAS_TRAIT(battlerDef, ABILITY_BULLETPROOF)
     && gMovesInfo[move].ballisticMove)
    {
        return TRUE;
    }
    if (AI_BATTLER_HAS_TRAIT(battlerDef, ABILITY_SAP_SIPPER)
     && moveType == TYPE_GRASS)
    {
        return TRUE;
    }
    ...
    if (gMovesInfo[move].power && !isDamageMoveUnusable)
    {
        ...
        if (GetActiveGimmick(battlerAtk) != GIMMICK_Z_MOVE)
        {
            ...
            case EFFECT_LEVEL_DAMAGE:
                simDamage.expected = simDamage.minimum = gBattleMons[battlerAtk].level * (AI_BATTLER_HAS_TRAIT(battlerAtk, ABILITY_PARENTAL_BOND) ? 2 : 1);
                break;
            case EFFECT_PSYWAVE:
                simDamage.expected = gBattleMons[battlerAtk].level * (AI_BATTLER_HAS_TRAIT(battlerAtk, ABILITY_PARENTAL_BOND) ? 2 : 1);
                simDamage.minimum = simDamage.expected / 2;
                break;
            case EFFECT_FIXED_DAMAGE_ARG:
                simDamage.expected = simDamage.minimum = gMovesInfo[move].argument * (AI_BATTLER_HAS_TRAIT(battlerAtk, ABILITY_PARENTAL_BOND) ? 2 : 1);
                break;
            case EFFECT_MULTI_HIT:
                ...
                else if (AI_BATTLER_HAS_TRAIT(battlerAtk, ABILITY_SKILL_LINK))
                {
            ...
            case EFFECT_SUPER_FANG:
                simDamage.expected = simDamage.minimum = (AI_BATTLER_HAS_TRAIT(battlerAtk, ABILITY_PARENTAL_BOND)
...
bool32 AI_IsBattlerGrounded(u32 battlerId)
{
    ...
    if (holdEffect == HOLD_EFFECT_IRON_BALL)
        ...
        return FALSE;
    else if (AI_BATTLER_HAS_TRAIT(battlerId, ABILITY_LEVITATE))
        return FALSE;
