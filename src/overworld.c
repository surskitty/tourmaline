...
u32 GetStationaryEncounterLevel(void) {
    ...
    if (FlagGet(FLAG_ENTERED_ELITE_FOUR)) {                 level = 60;
        ...
        if (FlagGet(FLAG_DEFEATED_MAGMA_SPACE_CENTER))      level += 2;
        if (FlagGet(FLAG_MET_TEAM_AQUA_HARBOR))             level += 2;
        if (FlagGet(FLAG_GROUDON_AWAKENED_MAGMA_HIDEOUT))   level += 2;
        if (FlagGet(FLAG_TEAM_AQUA_ESCAPED_IN_SUBMARINE))   level += 2;
        if (FlagGet(FLAG_BADGE06_GET))                      level += 3;
        if (FlagGet(FLAG_BADGE07_GET))                      level += 3;
        if (FlagGet(FLAG_BADGE08_GET))                      level += 3;
    } else if (FlagGet(FLAG_BADGE01_GET)) {             level = 10;
        if (FlagGet(FLAG_DELIVERED_STEVEN_LETTER))      level += 2;
        if (FlagGet(FLAG_BADGE02_GET))                  level += 2;
        if (FlagGet(FLAG_DOCK_REJECTED_DEVON_GOODS))    level += 2;
        if (FlagGet(FLAG_BADGE03_GET))                  level += 3;
        if (FlagGet(FLAG_BADGE04_GET))                  level += 3;
        if (FlagGet(FLAG_MET_ARCHIE_METEOR_FALLS))      level += 2;
    } else {
        if (FlagGet(FLAG_VISITED_RUSTBORO_CITY))        level = 7; 
