...
DOUBLE_BATTLE_TEST("INNATE: Aroma Veil protects the Pokémon's side from Taunt")
{
    ...
    GIVEN {
        ASSUME(GetMoveEffect(MOVE_TAUNT) == EFFECT_TAUNT);
        ASSUME(GetMoveCategory(MOVE_HARDEN) == DAMAGE_CATEGORY_STATUS);
        PLAYER(SPECIES_AROMATISSE) { Ability(ABILITY_LIGHT_METAL); Innates(ABILITY_AROMA_VEIL); }
        PLAYER(SPECIES_WOBBUFFET);
        OPPONENT(SPECIES_WOBBUFFET);
        OPPONENT(SPECIES_WYNAUT);
    } WHEN {
...
DOUBLE_BATTLE_TEST("INNATE: Aroma Veil protects the Pokémon's side from Cursed Body")
{
    GIVEN {
        ...
        PLAYER(SPECIES_AROMATISSE) { Ability(ABILITY_LIGHT_METAL); Innates(ABILITY_AROMA_VEIL); }
        PLAYER(SPECIES_WOBBUFFET);
        OPPONENT(SPECIES_JELLICENT) { Ability(ABILITY_LIGHT_METAL); Innates(ABILITY_CURSED_BODY); }
        OPPONENT(SPECIES_WYNAUT);
    } WHEN {
        TURN { MOVE(playerLeft, MOVE_PECK, target: opponentLeft); MOVE(playerRight, MOVE_PECK, target: opponentLeft); }
        TURN { MOVE(playerLeft, MOVE_PECK, target: opponentLeft); MOVE(playerRight, MOVE_PECK, target: opponentLeft); }
...
DOUBLE_BATTLE_TEST("INNATE: Aroma Veil protects the Pokémon's side from Heal Block")
{
    ...
    GIVEN {
        ASSUME(GetMoveEffect(MOVE_HEAL_BLOCK) == EFFECT_HEAL_BLOCK);
        ASSUME(GetMoveEffect(MOVE_RECOVER) == EFFECT_RESTORE_HP);
        PLAYER(SPECIES_AROMATISSE) { Ability(ABILITY_LIGHT_METAL); Innates(ABILITY_AROMA_VEIL); HP(1); }
        PLAYER(SPECIES_WOBBUFFET) { HP(1); }
        OPPONENT(SPECIES_WOBBUFFET);
        OPPONENT(SPECIES_WYNAUT);
    } WHEN {
