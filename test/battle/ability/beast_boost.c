...
SINGLE_BATTLE_TEST("Beast Boost boosts the most proficient stat when knocking out a target (Trait)")
{
    u8 stats[] = {1, 1, 1, 1, 1};
    PARAMETRIZE { stats[0] = 255; }
    PARAMETRIZE { stats[1] = 255; }
...
SINGLE_BATTLE_TEST("Beast Boost doesn't trigger if user is fainted (Trait)")
{
    GIVEN {
        ASSUME(GetMoveEffect(MOVE_DESTINY_BOND) == EFFECT_DESTINY_BOND);
        ASSUME(GetMovePower(MOVE_SCRATCH) > 0);
...
SINGLE_BATTLE_TEST("Beast Boost prioritizes stats in the case of a tie in the following order: Atk, Def, Sp.Atk, Sp.Def, Speed (Trait)")
{
    u8 stats[] = {1, 1, 1, 1, 1};
...
TO_DO_BATTLE_TEST("Beast Boost considers Power Split (Trait)");
TO_DO_BATTLE_TEST("Beast Boost considers Guard Split (Trait)");
TO_DO_BATTLE_TEST("Beast Boost considers Power Trick (Trait)");
TO_DO_BATTLE_TEST("Beast Boost considers Wonder Room (Trait)");
TO_DO_BATTLE_TEST("Beast Boost considers Speed Swap (Trait)");
TO_DO_BATTLE_TEST("Beast Boost doesn't consider stat stages (Trait)");
TO_DO_BATTLE_TEST("Beast Boost doesn't consider held items (Trait)");
TO_DO_BATTLE_TEST("Beast Boost doesn't consider status condition reductions (Trait)");
