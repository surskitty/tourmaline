...
SINGLE_BATTLE_TEST("INNATE: Cud Chew will activate Kee Berry effect again on the next turn")
{
    GIVEN {
        ...
        ASSUME(GetMoveCategory(MOVE_TACKLE) == DAMAGE_CATEGORY_PHYSICAL);
        PLAYER(SPECIES_WOBBUFFET);
        OPPONENT(SPECIES_TAUROS_PALDEA_COMBAT) { Ability(ABILITY_LIGHT_METAL); Innates(ABILITY_CUD_CHEW); Item(ITEM_KEE_BERRY); }
    } WHEN {
        TURN { MOVE(player, MOVE_TACKLE); }
        TURN { MOVE(player, MOVE_CELEBRATE);}
    } SCENE {
...
SINGLE_BATTLE_TEST("INNATE: Cud Chew will activate Oran Berry effect again on the next turn")
{
    GIVEN {
        ...
        ASSUME(GetMoveFixedDamage(MOVE_DRAGON_RAGE) == 40);
        PLAYER(SPECIES_WOBBUFFET);
        OPPONENT(SPECIES_TAUROS_PALDEA_COMBAT) { MaxHP(60); HP(60); Ability(ABILITY_LIGHT_METAL); Innates(ABILITY_CUD_CHEW); Item(ITEM_ORAN_BERRY); }
    } WHEN {
        TURN { MOVE(player, MOVE_DRAGON_RAGE); }
        TURN { MOVE(player, MOVE_CELEBRATE);}
        TURN {}
