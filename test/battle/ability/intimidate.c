...
SINGLE_BATTLE_TEST("ABILITY: Intimidate (opponent) lowers player's attack after switch out", s16 damage)
{
    u32 ability;
...
SINGLE_BATTLE_TEST("ABILITY: Intimidate (opponent) lowers player's attack after KO", s16 damage)
{
    u32 ability;
    PARAMETRIZE { ability = ABILITY_INTIMIDATE; }
    PARAMETRIZE { ability = ABILITY_SHED_SKIN; }
...
DOUBLE_BATTLE_TEST("ABILITY: Intimidate doesn't activate on an empty field in a double battle")
{
    GIVEN {
...
SINGLE_BATTLE_TEST("ABILITY: Intimidate and Eject Button force the opponent to Attack")
{
    GIVEN {
...
DOUBLE_BATTLE_TEST("ABILITY: Intimidate activates on an empty slot")
{
    GIVEN {
        PLAYER(SPECIES_WOBBUFFET);
...
DOUBLE_BATTLE_TEST("ABILITY: Intimidate activates immediately after the mon was switched in as long as one opposing mon is alive")
{
    GIVEN {
...
SINGLE_BATTLE_TEST("ABILITY: Intimidate can not further lower opponents Atk stat if it is at minimum stages")
{
    GIVEN {
        PLAYER(SPECIES_WOBBUFFET);
        OPPONENT(SPECIES_WOBBUFFET);
...
DOUBLE_BATTLE_TEST("ABILITY: Intimidate is not going to trigger if a mon switches out through u-turn and the opposing field is empty")
{
    GIVEN {
        PLAYER(SPECIES_WYNAUT);
        PLAYER(SPECIES_WOBBUFFET);
...
SINGLE_BATTLE_TEST("ABILITY: Intimidate activates when it's no longer effected by Neutralizing Gas - switching out")
{
    GIVEN {
...
SINGLE_BATTLE_TEST("ABILITY: Intimidate activates when it's no longer affected by Neutralizing Gas - switching moves")
{
    u32 move;
...
SINGLE_BATTLE_TEST("ABILITY: Intimidate activates when it's no longer affected by Neutralizing Gas - opponent caused switches")
{
    u32 move, item;
...
SINGLE_BATTLE_TEST("ABILITY: Intimidate activates when it's no longer affected by Neutralizing Gas - fainted")
{
    GIVEN {
...
DOUBLE_BATTLE_TEST("ABILITY: Intimidate will correctly decrease the attack of the second mon after Protosynthesis activated")
{
    GIVEN {
...
SINGLE_BATTLE_TEST("ABILITY: Intimdate does not lose timing after mega evolution and switch out by a hit escape move")
{
    GIVEN {
...
SINGLE_BATTLE_TEST("INNATE: Intimidate (opponent) lowers player's attack after switch out", s16 damage)
{
    u32 ability;
    PARAMETRIZE { ability = ABILITY_INTIMIDATE; }
    PARAMETRIZE { ability = ABILITY_SHED_SKIN; }
...
SINGLE_BATTLE_TEST("INNATE: Intimidate (opponent) lowers player's attack after KO", s16 damage)
{
    u32 ability;
    PARAMETRIZE { ability = ABILITY_INTIMIDATE; }
    PARAMETRIZE { ability = ABILITY_SHED_SKIN; }
    GIVEN {
        PLAYER(SPECIES_WOBBUFFET) { Speed(2); }
        OPPONENT(SPECIES_WOBBUFFET) { HP(1); Speed(1); }
        OPPONENT(SPECIES_ARBOK) { Ability(ABILITY_SHED_SKIN); Innates(ability); Speed(1); }
    } WHEN {
        TURN { MOVE(player, MOVE_TACKLE); SEND_OUT(opponent, 1); }
        TURN { MOVE(player, MOVE_TACKLE); }
    } SCENE {
...
DOUBLE_BATTLE_TEST("INNATE: Intimidate doesn't activate on an empty field in a double battle")
{
    GIVEN {
        ASSUME(GetMoveEffect(MOVE_EXPLOSION) == EFFECT_EXPLOSION);
        PLAYER(SPECIES_WOBBUFFET);
...
SINGLE_BATTLE_TEST("INNATE: Intimidate and Eject Button force the opponent to Attack")
{
    GIVEN {
        ASSUME(gItemsInfo[ITEM_EJECT_BUTTON].holdEffect == HOLD_EFFECT_EJECT_BUTTON);
        PLAYER(SPECIES_WOBBUFFET);
...
DOUBLE_BATTLE_TEST("INNATE: Intimidate activates on an empty slot")
{
    GIVEN {
        PLAYER(SPECIES_WOBBUFFET);
        PLAYER(SPECIES_CROAGUNK);
        PLAYER(SPECIES_WYNAUT);
        PLAYER(SPECIES_HITMONTOP) { Ability(ABILITY_STEADFAST); Innates(ABILITY_INTIMIDATE); }
        OPPONENT(SPECIES_RALTS);
        OPPONENT(SPECIES_AZURILL);
    } WHEN {
        TURN {
...
DOUBLE_BATTLE_TEST("INNATE: Intimidate activates immediately after the mon was switched in as long as one opposing mon is alive")
{
    GIVEN {
...
SINGLE_BATTLE_TEST("INNATE: Intimidate can not further lower opponents Atk stat if it is at minimum stages")
{
    GIVEN {
        PLAYER(SPECIES_WOBBUFFET);
        OPPONENT(SPECIES_WOBBUFFET);
...
DOUBLE_BATTLE_TEST("INNATE: Intimidate is not going to trigger if a mon switches out through u-turn and the opposing field is empty")
{
    GIVEN {
        PLAYER(SPECIES_WYNAUT);
        PLAYER(SPECIES_WOBBUFFET);
...
SINGLE_BATTLE_TEST("INNATE: Intimidate activates when it's no longer effected by Neutralizing Gas - switching out")
{
    GIVEN {
...
SINGLE_BATTLE_TEST("INNATE: Intimidate activates when it's no longer affected by Neutralizing Gas - switching moves")
{
    u32 move;
    PARAMETRIZE { move = MOVE_U_TURN; }
    PARAMETRIZE { move = MOVE_HEALING_WISH; }
...
SINGLE_BATTLE_TEST("INNATE: Intimidate activates when it's no longer affected by Neutralizing Gas - opponent caused switches")
{
    u32 move, item;
    PARAMETRIZE { move = MOVE_TACKLE; item = ITEM_EJECT_BUTTON; }
    PARAMETRIZE { move = MOVE_GROWL; item = ITEM_EJECT_PACK; }
    ...
    GIVEN {
        ...
        ASSUME(GetMoveEffect(MOVE_ROAR) == EFFECT_ROAR);
        ASSUME(GetMoveEffect(MOVE_DRAGON_TAIL) == EFFECT_HIT_SWITCH_TARGET);
        PLAYER(SPECIES_WEEZING) { Ability(ABILITY_LEVITATE); Innates(ABILITY_NEUTRALIZING_GAS); Item(item); }
        PLAYER(SPECIES_WOBBUFFET);
        OPPONENT(SPECIES_ARBOK) { Ability(ABILITY_INTIMIDATE); }
    } WHEN {
        if (item != ITEM_NONE) {
...
SINGLE_BATTLE_TEST("INNATE: Intimidate activates when it's no longer affected by Neutralizing Gas - fainted")
{
    GIVEN {
        ASSUME(GetMoveEffect(MOVE_FELL_STINGER) == EFFECT_FELL_STINGER);
        PLAYER(SPECIES_WEEZING) { Ability(ABILITY_LEVITATE); Innates(ABILITY_NEUTRALIZING_GAS); HP(1); }
        PLAYER(SPECIES_WOBBUFFET);
        OPPONENT(SPECIES_ARBOK) { Ability(ABILITY_INTIMIDATE); }
    } WHEN {
        TURN { MOVE(opponent, MOVE_FELL_STINGER); SEND_OUT(player, 1); }
...
DOUBLE_BATTLE_TEST("INNATE: Intimidate will correctly decrease the attack of the second mon after Protosynthesis activated")
{
    GIVEN {
        PLAYER(SPECIES_WOBBUFFET) { Item(ITEM_EJECT_PACK); }
        PLAYER(SPECIES_WYNAUT);
        PLAYER(SPECIES_WALKING_WAKE) { Ability(ABILITY_SHED_SKIN); Innates(ABILITY_PROTOSYNTHESIS); Item(ITEM_BOOSTER_ENERGY); }
        OPPONENT(SPECIES_WYNAUT);
        OPPONENT(SPECIES_WOBBUFFET);
...
SINGLE_BATTLE_TEST("INNATE: Intimdate does not lose timing after mega evolution and switch out by a hit escape move")
{
    GIVEN {
        ASSUME(gMovesInfo[MOVE_U_TURN].effect == EFFECT_HIT_ESCAPE);
        PLAYER(SPECIES_WOBBUFFET);
