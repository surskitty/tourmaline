...
SINGLE_BATTLE_TEST("ABILITY: Liquid Ooze causes Absorb users to lose HP instead of heal")
{
    s16 damage;
    s16 healed;
    GIVEN {
...
SINGLE_BATTLE_TEST("ABILITY: Liquid Ooze causes Leech Seed users to lose HP instead of heal")
{
    s16 damage;
    s16 healed;
...
DOUBLE_BATTLE_TEST("ABILITY: Liquid Ooze causes Matcha Gatcha users to lose HP instead of heal")
{
    GIVEN {
...
DOUBLE_BATTLE_TEST("ABILITY: Liquid Ooze will faint Matcha Gatcha users if it deals enough damage")
{
    GIVEN {
        ASSUME(GetMoveEffect(MOVE_MATCHA_GOTCHA) == EFFECT_ABSORB);
        PLAYER(SPECIES_WOBBUFFET) { HP(1); }
...
SINGLE_BATTLE_TEST("ABILITY: Liquid Ooze causes Strength Sap users to lose HP instead of heal")
{
    s16 lostHp;
    s32 atkStat;
 ...
 */
SINGLE_BATTLE_TEST("ABILITY: Liquid Ooze causes leech seed victim to faint before seeder")
{
    u16 ability;
...
TO_DO_BATTLE_TEST("ABILITY: Liquid Ooze does not cause Dream Eater users to lose HP instead of heal (Gen 3-4");
TO_DO_BATTLE_TEST("ABILITY: Liquid Ooze causes Dream Eater users to lose HP instead of heal (Gen 5+");

SINGLE_BATTLE_TEST("INNATE: Liquid Ooze causes Absorb users to lose HP instead of heal")
{
    s16 damage;
    s16 healed;
    GIVEN {
...
SINGLE_BATTLE_TEST("INNATE: Liquid Ooze causes Leech Seed users to lose HP instead of heal")
{
    s16 damage;
    s16 healed;
...
DOUBLE_BATTLE_TEST("INNATE: Liquid Ooze causes Matcha Gatcha users to lose HP instead of heal")
{
    GIVEN {
        ASSUME(GetMoveEffect(MOVE_MATCHA_GOTCHA) == EFFECT_ABSORB);
        PLAYER(SPECIES_WOBBUFFET) { HP(1); }
...
DOUBLE_BATTLE_TEST("INNATE: Liquid Ooze will faint Matcha Gatcha users if it deals enough damage")
{
    GIVEN {
        ASSUME(GetMoveEffect(MOVE_MATCHA_GOTCHA) == EFFECT_ABSORB);
        PLAYER(SPECIES_WOBBUFFET) { HP(1); }
...
SINGLE_BATTLE_TEST("INNATE: Liquid Ooze causes Strength Sap users to lose HP instead of heal")
{
    s16 lostHp;
    s32 atkStat;
...
/* * https://bulbapedia.bulbagarden.net/wiki/Liquid_Ooze_(Ability)#In_battle:
   * If the recipient of Leech Seed's effect were to faint due to Liquid Ooze on the same turn as the victim of Leech Seed, then the victim faints before the recipient. This means that the victim's team loses the battle if both teams had their final Pokémon sent out. 
 */
SINGLE_BATTLE_TEST("INNATE: Liquid Ooze causes leech seed victim to faint before seeder")
{
    u16 ability;
    PARAMETRIZE { ability = ABILITY_CLEAR_BODY; }
    PARAMETRIZE { ability = ABILITY_LIQUID_OOZE; }
...
TO_DO_BATTLE_TEST("INNATE: Liquid Ooze does not cause Dream Eater users to lose HP instead of heal (Gen 3-4");
TO_DO_BATTLE_TEST("INNATE: Liquid Ooze causes Dream Eater users to lose HP instead of heal (Gen 5+");
