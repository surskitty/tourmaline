...
SINGLE_BATTLE_TEST("ABILITY: Parental Bond converts Tackle into a two-strike move")
{
    GIVEN {
...
SINGLE_BATTLE_TEST("ABILITY: Parental Bond does not convert a move with three or more strikes to a two-strike move")
{
    GIVEN {
...
SINGLE_BATTLE_TEST("ABILITY: Parental Bond converts multi-target moves into a two-strike move in Single Battles")
{
    u16 move;
...
DOUBLE_BATTLE_TEST("ABILITY: Parental Bond does not convert multi-target moves into a two-strike move in Double Battles, even if it only damages one")
{
    GIVEN {
        ASSUME(GetMoveStrikeCount(MOVE_EARTHQUAKE) < 2);
        ASSUME(GetMoveTarget(MOVE_EARTHQUAKE) == MOVE_TARGET_FOES_AND_ALLY);
...
SINGLE_BATTLE_TEST("ABILITY: Parental Bond-converted moves only hit once on Lightning Rod/Storm Drain mons")
{
...
SINGLE_BATTLE_TEST("ABILITY: Parental Bond has no affect on multi hit moves and they still hit twice 37.5/35% of the time")
{
...
SINGLE_BATTLE_TEST("ABILITY: Parental Bond has no affect on multi hit moves and they still hit thrice 37.5/35% of the time")
{
    u32 genConfig, passes, trials;
    PARAMETRIZE { genConfig = GEN_4; passes = 3; trials = 8; }  // 37.5%
    PARAMETRIZE { genConfig = GEN_5; passes = 7; trials = 20; } // 35%
...
SINGLE_BATTLE_TEST("ABILITY: Parental Bond has no affect on multi hit moves and they still hit four times 12.5/15% of the time")
{
    u32 genConfig, passes, trials;
...
SINGLE_BATTLE_TEST("ABILITY: Parental Bond has no affect on multi hit moves and they still hit five times 12.5/15% of the time")
{
    u32 genConfig, passes, trials;
    PARAMETRIZE { genConfig = GEN_4; passes = 1; trials = 8; }  // 12.5%
    PARAMETRIZE { genConfig = GEN_5; passes = 3; trials = 20; } // 15%
...
SINGLE_BATTLE_TEST("ABILITY: Parental Bond Smack Down effect triggers after 2nd hit")
{
    GIVEN {
...
SINGLE_BATTLE_TEST("ABILITY: Parental Bond Snore strikes twice while asleep")
{
    s16 damage[2];
    GIVEN {
...
SINGLE_BATTLE_TEST("ABILITY: Parental Bond only triggers Dragon Tail's target switch out on the second hit")
{
    GIVEN {
...
SINGLE_BATTLE_TEST("ABILITY: Parental Bond does not trigger on semi-invulnerable moves")
{
    GIVEN {
...
SINGLE_BATTLE_TEST("ABILITY: Parental Bond does not trigger on two turn attacks")
{
    GIVEN {
...
TO_DO_BATTLE_TEST("ABILITY: Parental Bond tests");
...
SINGLE_BATTLE_TEST("INNATE: Parental Bond converts Tackle into a two-strike move")
{
    GIVEN {
        ASSUME(GetMoveCategory(MOVE_TACKLE) != DAMAGE_CATEGORY_STATUS);
        ASSUME(GetMoveStrikeCount(MOVE_TACKLE) < 2);
...
SINGLE_BATTLE_TEST("INNATE: Parental Bond does not convert a move with three or more strikes to a two-strike move")
{
    GIVEN {
        ASSUME(GetMoveCategory(MOVE_TRIPLE_KICK) != DAMAGE_CATEGORY_STATUS);
        ASSUME(GetMoveStrikeCount(MOVE_TRIPLE_KICK) == 3);
...
SINGLE_BATTLE_TEST("INNATE: Parental Bond converts multi-target moves into a two-strike move in Single Battles")
{
    u16 move;
    PARAMETRIZE { move = MOVE_EARTHQUAKE; }
    PARAMETRIZE { move = MOVE_ICY_WIND; }
...
DOUBLE_BATTLE_TEST("INNATE: Parental Bond does not convert multi-target moves into a two-strike move in Double Battles, even if it only damages one")
{
    GIVEN {
        ASSUME(GetMoveStrikeCount(MOVE_EARTHQUAKE) < 2);
        ASSUME(GetMoveTarget(MOVE_EARTHQUAKE) == MOVE_TARGET_FOES_AND_ALLY);
...
SINGLE_BATTLE_TEST("INNATE: Parental Bond-converted moves only hit once on Lightning Rod/Storm Drain mons")
{
    u16 move, species, ability, type;
    PARAMETRIZE { move = MOVE_THUNDERBOLT; ability = ABILITY_LIGHTNING_ROD; species = SPECIES_RAICHU; type = TYPE_ELECTRIC; }
    PARAMETRIZE { move = MOVE_SURF; ability = ABILITY_STORM_DRAIN; species = SPECIES_LILEEP; type = TYPE_WATER; }
...
SINGLE_BATTLE_TEST("INNATE: Parental Bond has no affect on multi hit moves and they still hit twice 37.5/35% of the time")
{
    u32 genConfig, passes, trials;
    PARAMETRIZE { genConfig = GEN_4; passes = 3; trials = 8; }  // 37.5%
    PARAMETRIZE { genConfig = GEN_5; passes = 7; trials = 20; } // 35%
...
SINGLE_BATTLE_TEST("INNATE: Parental Bond has no affect on multi hit moves and they still hit thrice 37.5/35% of the time")
{
    u32 genConfig, passes, trials;
    PARAMETRIZE { genConfig = GEN_4; passes = 3; trials = 8; }  // 37.5%
    PARAMETRIZE { genConfig = GEN_5; passes = 7; trials = 20; } // 35%
...
SINGLE_BATTLE_TEST("INNATE: Parental Bond has no affect on multi hit moves and they still hit four times 12.5/15% of the time")
{
    u32 genConfig, passes, trials;
    PARAMETRIZE { genConfig = GEN_4; passes = 1; trials = 8; }  // 12.5%
    PARAMETRIZE { genConfig = GEN_5; passes = 3; trials = 20; } // 15%
...
SINGLE_BATTLE_TEST("INNATE: Parental Bond has no affect on multi hit moves and they still hit five times 12.5/15% of the time")
{
    u32 genConfig, passes, trials;
    PARAMETRIZE { genConfig = GEN_4; passes = 1; trials = 8; }  // 12.5%
    PARAMETRIZE { genConfig = GEN_5; passes = 3; trials = 20; } // 15%
...
SINGLE_BATTLE_TEST("INNATE: Parental Bond Smack Down effect triggers after 2nd hit")
{
    GIVEN {
        ASSUME(GetMoveCategory(MOVE_SMACK_DOWN) != DAMAGE_CATEGORY_STATUS);
        ASSUME(GetMoveStrikeCount(MOVE_SMACK_DOWN) < 2);
...
SINGLE_BATTLE_TEST("INNATE: Parental Bond Snore strikes twice while asleep")
{
    s16 damage[2];
    GIVEN {
        ASSUME(GetMoveEffect(MOVE_SNORE) == EFFECT_SNORE);
...
SINGLE_BATTLE_TEST("INNATE: Parental Bond only triggers Dragon Tail's target switch out on the second hit")
{
    GIVEN {
        ASSUME(GetMoveEffect(MOVE_DRAGON_TAIL) == EFFECT_HIT_SWITCH_TARGET);
        PLAYER(SPECIES_KANGASKHAN) { Item(ITEM_KANGASKHANITE); }
...
SINGLE_BATTLE_TEST("INNATE: Parental Bond does not trigger on semi-invulnerable moves")
{
    GIVEN {
        ASSUME(GetMoveCategory(MOVE_FLY) != DAMAGE_CATEGORY_STATUS);
        ASSUME(GetMoveStrikeCount(MOVE_FLY) < 2);
...
SINGLE_BATTLE_TEST("INNATE: Parental Bond does not trigger on two turn attacks")
{
    GIVEN {
        ASSUME(GetMoveCategory(MOVE_RAZOR_WIND) != DAMAGE_CATEGORY_STATUS);
        ASSUME(GetMoveStrikeCount(MOVE_RAZOR_WIND) < 2);
...
TO_DO_BATTLE_TEST("INNATE: Parental Bond tests");
