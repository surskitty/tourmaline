...
DOUBLE_BATTLE_TEST("INNATE: Supreme Overlord boosts Attack by an additive 10% per fainted mon on its side upon switch in", s16 damage)
{
    bool32 switchMon = 0;
    PARAMETRIZE { switchMon = FALSE; }
    PARAMETRIZE { switchMon = TRUE; }
    GIVEN {
        PLAYER(SPECIES_KINGAMBIT) { Ability(ABILITY_LIGHT_METAL); Innates(ABILITY_SUPREME_OVERLORD); }
        PLAYER(SPECIES_PAWNIARD);
        PLAYER(SPECIES_PAWNIARD);
        PLAYER(SPECIES_PAWNIARD);
        OPPONENT(SPECIES_WOBBUFFET);
...
DOUBLE_BATTLE_TEST("INNATE: Supreme Overlord's boost caps at a 1.5x multipler", s16 damage)
{
    u32 faintCount = 0;
    PARAMETRIZE { faintCount = 5; }
    PARAMETRIZE { faintCount = 6; }
...
SINGLE_BATTLE_TEST("INNATE: Supreme Overlord does not boost attack if party members are already fainted at the start of the battle", s16 damage)
{
    u32 fainted = 0;
...
SINGLE_BATTLE_TEST("INNATE: Supreme Overlord's message displays correctly after all battlers fainted - Player")
{
    GIVEN {
        ASSUME(GetMoveEffect(MOVE_EXPLOSION) == EFFECT_EXPLOSION);
        PLAYER(SPECIES_WOBBUFFET) { HP(1);}
...
SINGLE_BATTLE_TEST("INNATE: Supreme Overlord's message displays correctly after all battlers fainted - Opponent")
{
    GIVEN {
        ASSUME(GetMoveEffect(MOVE_EXPLOSION) == EFFECT_EXPLOSION);
        PLAYER(SPECIES_WOBBUFFET);
        PLAYER(SPECIES_WOBBUFFET);
        OPPONENT(SPECIES_WOBBUFFET) { HP(1);}
        OPPONENT(SPECIES_KINGAMBIT) { Ability(ABILITY_LIGHT_METAL); Innates(ABILITY_SUPREME_OVERLORD); }
    } WHEN {
        TURN { MOVE(player, MOVE_EXPLOSION); SEND_OUT(player, 1); SEND_OUT(opponent, 1); }
    } SCENE {
        HP_BAR(player, hp: 0);
