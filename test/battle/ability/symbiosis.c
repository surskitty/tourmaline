...
DOUBLE_BATTLE_TEST("INNATE: Symbiosis transfers its item to an ally after it consumes an item")
{
    GIVEN {
        ASSUME(gItemsInfo[ITEM_ROOM_SERVICE].holdEffect == HOLD_EFFECT_ROOM_SERVICE);
        PLAYER(SPECIES_WOBBUFFET) { Item(ITEM_ROOM_SERVICE); }
        PLAYER(SPECIES_ORANGURU) { Ability(ABILITY_LIGHT_METAL); Innates(ABILITY_SYMBIOSIS); Item(ITEM_TOXIC_ORB); }
        OPPONENT(SPECIES_KIRLIA);
        OPPONENT(SPECIES_SHUCKLE);
    } WHEN {
        TURN { MOVE(opponentLeft, MOVE_TRICK_ROOM); }
...
DOUBLE_BATTLE_TEST("INNATE: Symbiosis triggers after partner bestows its item")
{
    GIVEN {
        PLAYER(SPECIES_WOBBUFFET) { Speed(100); Item(ITEM_FLAME_ORB); }
        PLAYER(SPECIES_ORANGURU) { Speed(75); Ability(ABILITY_LIGHT_METAL); Innates(ABILITY_SYMBIOSIS); Item(ITEM_TOXIC_ORB); }
        OPPONENT(SPECIES_STARAVIA) { Speed(50); }
        OPPONENT(SPECIES_SHUCKLE) { Speed(25); }
    } WHEN {
        TURN { MOVE(playerLeft, MOVE_BESTOW, target: opponentLeft); }
