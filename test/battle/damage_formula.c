...
DOUBLE_BATTLE_TEST("INNATE: Transistor Damage calculation", s16 damage)
{
    ...
    GIVEN {
        ...
        PLAYER(SPECIES_REGIELEKI) { Ability(ABILITY_LIGHT_METAL); Innates(ABILITY_KLUTZ); }
        PLAYER(SPECIES_REGIELEKI) { Ability(ABILITY_LIGHT_METAL); Innates(ABILITY_TRANSISTOR); }
        OPPONENT(SPECIES_REGIELEKI) { Ability(ABILITY_LIGHT_METAL); Innates(ABILITY_KLUTZ); }
        OPPONENT(SPECIES_REGIELEKI) { Ability(ABILITY_LIGHT_METAL); Innates(ABILITY_TRANSISTOR); }
    } WHEN {
        TURN {
            MOVE(playerLeft, MOVE_THUNDER_SHOCK, target: opponentLeft, WITH_RNG(RNG_DAMAGE_MODIFIER, 15 - i));
            MOVE(playerRight, MOVE_THUNDER_SHOCK, target: opponentRight, WITH_RNG(RNG_DAMAGE_MODIFIER, 15 - i));
