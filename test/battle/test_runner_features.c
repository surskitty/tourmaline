...
SINGLE_BATTLE_TEST("INNATE: Forced abilities activate on switch-in")
{
    GIVEN {
        PLAYER(SPECIES_ALAKAZAM);
        PLAYER(SPECIES_KADABRA) { Ability(ABILITY_LIGHT_METAL); Innates(ABILITY_QUARK_DRIVE); SpAttack(400);}
        OPPONENT(SPECIES_ARON);
        OPPONENT(SPECIES_ALAKAZAM) { Ability(ABILITY_LIGHT_METAL); Innates(ABILITY_ELECTRIC_SURGE); };
    } WHEN {
        TURN { SWITCH(player, 1); SWITCH(opponent, 1);}
    } SCENE {
        ABILITY_POPUP(opponent, ABILITY_ELECTRIC_SURGE);
